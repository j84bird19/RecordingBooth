(() => {
  ...
  const VERSION = 'fresh-v1-logic-daw-core';
  const DB_NAME = 'recording-booth-fresh-v1';
    ...
    tracks.forEach(t => {
      ...
      card.innerHTML = `
        ...
        <div class="track-lane">
          ...
          <div class="fx-row">
            <label>EQ <input type="checkbox" data-action="fx" data-fx="eq"></label>
            <label>Comp <input type="checkbox" data-action="fx" data-fx="comp"></label>
            <label>Rev <input type="range" min="0" max="1" step="0.01" value="0"></label>
          </div>
        </div>
      `;
  ...
  function scheduleSources() {
    ...
    audibleTracks().forEach(t => {
      ...
      const inputGain = engine.audioCtx.createGain();
      const outputGain = engine.audioCtx.createGain();
      const pan = engine.audioCtx.createStereoPanner ? engine.audioCtx.createStereoPanner() : null;
      inputGain.gain.value = t.gain;
      outputGain.gain.value = t.volume;
      source.connect(inputGain);
      if (pan) {
        pan.pan.value = t.pan;
        inputGain.connect(pan); pan.connect(outputGain);
      } else {
      ...
      t.nodes = { source, inputGain, outputGain, pan };
  ...
  function exportRoughMix() {
    ...
    downloadBlob(encodeWavBlob(fake), `${safeName(els.songName.value || 'MicCheckSong')}_rough_mix.wav`);
    ...
    try {
      ...
      const data = {
        ...
        tracks: tracks.map(t => ({
          ...
          muted: t.muted, solo: t.solo, armed: false, type: t.type, blob: t.blob || null
        }))
      };
  ...
  async function loadProject() {
    try {
      ...
      for (const saved of data.tracks || []) {
        ...
        Object.assign(t, { fileName: saved.fileName || '', startTime: saved.startTime || 0, volume: saved.volume ?? 1, pan: saved.pan ?? 0, gain: saved.gain ?? 1, muted: !!saved.muted, solo: !!saved.solo, armed: false, blob: saved.blob || null, audioBuffer: null });
        if (saved.blob) {
  ...
  els.exportMix.addEventListener('click', exportRoughMix);
