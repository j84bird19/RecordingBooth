(() => {
  ...
  const VERSION = 'fresh-v1-6-headset-playback-fix';
      ...
      card.innerHTML = `
        <div class="track-head">
          ...
          <div class="control-row">
            ...
            ${t.type === 'vocal' ? `<button class="btn tiny toggle arm" data-action="arm">R</button><button class="btn tiny danger" data-action="clear">Clear</button>` : ''}
          </div>
  ...
  function updateFxNodes(track) {
    ...
    if (track.nodes.highpass) track.nodes.highpass.frequency.setTargetAtTime(Math.max(20, Number(fx.lowCut) || 80), now, 0.015);
    if (track.nodes.presence) track.nodes.presence.gain.setTargetAtTime(Number(fx.presence) || 0, now, 0.015);
    if (track.nodes.air) track.nodes.air.gain.setTargetAtTime(Number(fx.air) || 0, now, 0.015);
    if (track.nodes.compressor) {
      track.nodes.compressor.threshold.setTargetAtTime(-6 - comp * 34, now, 0.015);
      track.nodes.compressor.knee.setTargetAtTime(4 + comp * 22, now, 0.015);
      track.nodes.compressor.ratio.setTargetAtTime(1 + comp * 7, now, 0.015);
      track.nodes.compressor.attack.setTargetAtTime(0.004 + comp * 0.018, now, 0.015);
      track.nodes.compressor.release.setTargetAtTime(0.08 + comp * 0.28, now, 0.015);
    }
    if (track.nodes.reverbGain) track.nodes.reverbGain.gain.setTargetAtTime(Math.pow(rev, 1.15) * 0.85, now, 0.015);
    if (track.nodes.delayNode) track.nodes.delayNode.delayTime.setTargetAtTime(0.12 + del * 0.55, now, 0.015);
    if (track.nodes.delayWetGain) track.nodes.delayWetGain.gain.setTargetAtTime(Math.pow(del, 1.1) * 0.62, now, 0.015);
    if (track.nodes.feedbackGain) track.nodes.feedbackGain.gain.setTargetAtTime(del * 0.42, now, 0.015);
  }
